...
function ResetForm() {
  ...
  return <Card><CardHeader><CardTitle>Choose a new password</CardTitle></CardHeader><CardContent>{status === "success" ? <p className="text-sm">Password updated. <Link href="/account/login" className="font-semibold text-sky hover:underline">Sign in</Link>.</p> : <form onSubmit={submit} className="space-y-4"><Input name="password" label="New password" type="password" minLength={8} required /><Input name="confirm" label="Confirm password" type="password" minLength={8} required />{status === "error" && <p className="text-sm text-error">{error}</p>}<Button className="w-full">Update password</Button></form>}</CardContent></Card>;
}
